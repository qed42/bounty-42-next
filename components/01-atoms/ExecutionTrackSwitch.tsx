...
export default function ExecutionTrackSwitch({
  id,
  selected,
  onToggle,
}: ExecutionTrackSwitchProps) {
  ...
  return (
    <div
      className={`flex items-center gap-6 justify-between py-4 text-gray-700 ${
        selected ? " text-primary" : " "
      }`}
    >
      <span className="text-lg font-bold">
        Execution Plan: {selected ? "Selected" : "Not selected"}
      </span>
      <Switch
        checked={selected}
        onCheckedChange={handleToggle}
        disabled={isPending}
        className="scale-150 hover:cursor-pointer mr-2"
      />
    </div>
  );
}
